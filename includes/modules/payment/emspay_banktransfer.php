...
class emspay_banktransfer {
  ...
  // Class Constructor
  function emspay_banktransfer() {
    global $order;

    $this->code = 'emspay_banktransfer';
    $this->title_selection = MODULE_PAYMENT_EMSPAY_BANKTRANSFER_TEXT_TITLE;
    $this->title = 'EMS Online ' . $this->title_selection;
    $this->description = MODULE_PAYMENT_EMSPAY_BANKTRANSFER_TEXT_DESCRIPTION;
    $this->email_footer = MODULE_PAYMENT_EMSPAY_BANKTRANSFER_TEXT_EMAIL_FOOTER;
  ...
  function update_status() {
    ...
    if ( ( $this->enabled == true ) && ( (int)MODULE_PAYMENT_EMSPAY_BANKTRANSFER_ZONE > 0 ) ) {
      $check_flag = false;
      $check_query = tep_db_query( "select zone_id from " . TABLE_ZONES_TO_GEO_ZONES . " where geo_zone_id = '" . intval( MODULE_PAYMENT_EMSPAY_BANKTRANSFER_ZONE ) . "' and zone_country_id = '" . intval( $order->billing['country']['id'] ) . "' order by zone_id" );
  ...
  function confirmation() {
    return array('title' => MODULE_PAYMENT_EMSPAY_BANKTRANSFER_TEXT_DESCRIPTION);
  }
  ...
  function after_process() {
    global $insert_id, $order, $email_order;

    if ($this->after_process_has_run)
      return true;

    $this->after_process_has_run = true;
    ...
    tep_db_query( "update ". TABLE_ORDERS. " set orders_status = " . intval( MODULE_PAYMENT_EMSPAY_NEW_STATUS_ID ) . ", emspay_order_id = '" . $emspay_order['id']  . "', emspay_bank_reference = '" . $emspay_order['transactions'][0]['payment_method_details']['reference'] . "' where orders_id = ". intval( $insert_id ) );

    $this->email_footer = sprintf(MODULE_PAYMENT_EMSPAY_BANKTRANSFER_TEXT_EMAIL_FOOTER, $order->info['total'], $emspay_order['transactions'][0]['payment_method_details']['reference']);
  ...
  function check() {
    if ( !isset( $this->_check ) ) {
      $check_query = tep_db_query( "select configuration_value from " . TABLE_CONFIGURATION . " where configuration_key = 'MODULE_PAYMENT_EMSPAY_BANKTRANSFER_STATUS'" );
  ...
  function install() {
    ...
    // ADD THE EMSPAY BANK REFERENCE TO THE ORDERS TABLE
    if (!$this->tableColumnExists("orders", "emspay_bank_reference")) {
      if (!tep_db_query("ALTER TABLE orders ADD emspay_bank_reference VARCHAR( 16 ) NULL DEFAULT NULL ;")) {
        die("To be able to work; please add the column emspay_bank_reference (VARCHAR 16, DEFAULT NULL) to your order table");
      }
    }    
