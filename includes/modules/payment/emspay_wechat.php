...
class emspay_wechat {
  ...
  // Class Constructor
  function emspay_wechat() {
    global $order;

    $this->code = 'emspay_wechat';
    $this->id = 'wechat';
    $this->title_selection = MODULE_PAYMENT_EMSPAY_WECHAT_TEXT_TITLE;
    ...
    if ( (int)MODULE_PAYMENT_EMSPAY_ORDER_STATUS_ID > 0 ) {
      ...
      $payment = 'emspay_wechat';
    } else if ( $payment == 'emspay_wechat' ) {
      $payment = '';
    }
    if ( is_object( $order ) ) {
      $this->update_status();
  ...
  function update_status() {
    ...
    if ( ( $this->enabled == true ) && ( (int)MODULE_PAYMENT_EMSPAY_WECHAT_ZONE > 0 ) ) {
      $check_flag = false;
      $check_query = tep_db_query( "select zone_id from " . TABLE_ZONES_TO_GEO_ZONES . " where geo_zone_id = '" . intval( MODULE_PAYMENT_EMSPAY_WECHAT_ZONE ) . "' and zone_country_id = '" . intval( $order->billing['country']['id'] ) . "' order by zone_id" );
  ...
  function after_process() {
    ...
    if ( !is_array( $emspay_order ) or array_key_exists( 'error', $emspay_order ) or $emspay_order['status'] == 'error' ) {
      ...
      $this->tep_remove_order( $insert_id, $restock = true );
        $reason = "Error placing WeChat order ";
  ...
  function check() {
    if ( !isset( $this->_check ) ) {
      $check_query = tep_db_query( "select configuration_value from " . TABLE_CONFIGURATION . " where configuration_key = 'MODULE_PAYMENT_EMSPAY_WECHAT_STATUS'" );
  ...
  function install() {
    ...
    $sort_order = 0;
    $add_array = array(
        "configuration_title" => 'Enable EMS Online WeChat Module',
        "configuration_key" => 'MODULE_PAYMENT_EMSPAY_WECHAT_STATUS',
        "configuration_value" => 'False',
        "configuration_description" => 'Do you want to accept WeChat payments using EMS Online?',
