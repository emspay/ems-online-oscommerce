...
class emspay_paypal {
  ...
  // Class Constructor
  function emspay_paypal() {
    global $order;

    $this->code = 'emspay_paypal';
    $this->title_selection = MODULE_PAYMENT_EMSPAY_PAYPAL_TEXT_TITLE;
    $this->title = 'EMS Online ' . $this->title_selection;
    $this->description = MODULE_PAYMENT_EMSPAY_PAYPAL_TEXT_DESCRIPTION;
    $this->sort_order = MODULE_PAYMENT_EMSPAY_PAYPAL_SORT_ORDER;
    $this->enabled = ( ( MODULE_PAYMENT_EMSPAY_PAYPAL_STATUS == 'True' ) ? true : false );
    ...
    if ( (int)MODULE_PAYMENT_EMSPAY_ORDER_STATUS_ID > 0 ) {
      ...
      $payment = 'emspay_paypal';
    } else if ( $payment=='emspay_paypal' ) {
        $payment='';
      }
    if ( is_object( $order ) ) {
      $this->update_status();
  ...
  function update_status() {
    ...
    if ( ( $this->enabled == true ) && ( (int)MODULE_PAYMENT_EMSPAY_PAYPAL_ZONE > 0 ) ) {
      $check_flag = false;
      $check_query = tep_db_query( "select zone_id from " . TABLE_ZONES_TO_GEO_ZONES . " where geo_zone_id = '" . intval( MODULE_PAYMENT_EMSPAY_PAYPAL_ZONE ) . "' and zone_country_id = '" . intval( $order->billing['country']['id'] ) . "' order by zone_id" );
  ...
  function after_process() {
    ...
    $emspay_order = $this->emspay->emsCreatePaypalOrder( $insert_id, 
                                                        $order->info['total'], 
  ...
  function check() {
    if ( !isset( $this->_check ) ) {
      $check_query = tep_db_query( "select configuration_value from " . TABLE_CONFIGURATION . " where configuration_key = 'MODULE_PAYMENT_EMSPAY_PAYPAL_STATUS'" );
  ...
  function install() {
    ...
    $add_array = array(
      ...
      "configuration_key" => 'MODULE_PAYMENT_EMSPAY_PAYPAL_STATUS',
    ...
    $add_array = array(
      ...
      "configuration_key" => 'MODULE_PAYMENT_EMSPAY_PAYPAL_ZONE',
    ...
    $add_array = array(
      ...
      "configuration_key" => 'MODULE_PAYMENT_EMSPAY_PAYPAL_SORT_ORDER',
      "configuration_value" => 0,
  ...
  function keys() {
    return array(
      'MODULE_PAYMENT_EMSPAY_PAYPAL_STATUS',
      'MODULE_PAYMENT_EMSPAY_PAYPAL_ZONE',
      'MODULE_PAYMENT_EMSPAY_PAYPAL_SORT_ORDER',
    );
  }
