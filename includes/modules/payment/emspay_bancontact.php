...
class emspay_bancontact {
  ...
  // Class Constructor
  function emspay_bancontact() {
    global $order;

    $this->code = 'emspay_bancontact';
    $this->title_selection = MODULE_PAYMENT_EMSPAY_BANCONTACT_TEXT_TITLE;
    ...
    if ( (int)MODULE_PAYMENT_EMSPAY_ORDER_STATUS_ID > 0 ) {
      ...
      $payment = 'emspay_bancontact';
    } else if ( $payment=='emspay_bancontact' ) {
        $payment='';
      }
    if ( is_object( $order ) ) {
      $this->update_status();
  ...
  function update_status() {
    ...
    if ( ( $this->enabled == true ) && ( (int)MODULE_PAYMENT_EMSPAY_BANCONTACT_ZONE > 0 ) ) {
      $check_flag = false;
      $check_query = tep_db_query( "select zone_id from " . TABLE_ZONES_TO_GEO_ZONES . " where geo_zone_id = '" . intval( MODULE_PAYMENT_EMSPAY_BANCONTACT_ZONE ) . "' and zone_country_id = '" . intval( $order->billing['country']['id'] ) . "' order by zone_id" );
  ...
  function after_process() {
    ...
    $emspay_order = $this->emspay->emsCreateBcOrder( $insert_id, 
                                                     $order->info['total'], 
  ...
  function check() {
    if ( !isset( $this->_check ) ) {
      $check_query = tep_db_query( "select configuration_value from " . TABLE_CONFIGURATION . " where configuration_key = 'MODULE_PAYMENT_EMSPAY_BANCONTACT_STATUS'" );
