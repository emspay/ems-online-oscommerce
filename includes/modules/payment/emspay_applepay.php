...
class emspay_applepay {
  ...
  // Class Constructor
  function emspay_applepay() {
    global $order;

    $this->code = 'emspay_applepay';
    $this->title_selection = MODULE_PAYMENT_EMSPAY_APPLEPAY_TEXT_TITLE;
    $this->title = 'EMS Online ' . $this->title_selection;
    $this->description = MODULE_PAYMENT_EMSPAY_APPLEPAY_TEXT_DESCRIPTION;
    $this->sort_order = MODULE_PAYMENT_EMSPAY_APPLEPAY_SORT_ORDER;
    $this->enabled = ( ( MODULE_PAYMENT_EMSPAY_APPLEPAY_STATUS == 'True' ) ? true : false );
    ...
    if ( (int)MODULE_PAYMENT_EMSPAY_ORDER_STATUS_ID > 0 ) {
      ...
      $payment = 'emspay_applepay';
    } else if ( $payment=='emspay_applepay' ) {
        $payment='';
      }
    if ( is_object( $order ) ) {
      $this->update_status();
  ...
  function update_status() {
    ...
    if ( ( $this->enabled == true ) && ( (int)MODULE_PAYMENT_EMSPAY_APPLEPAY_ZONE > 0 ) ) {
      $check_flag = false;
      $check_query = tep_db_query( "select zone_id from " . TABLE_ZONES_TO_GEO_ZONES . " where geo_zone_id = '" . intval( MODULE_PAYMENT_EMSPAY_APPLEPAY_ZONE ) . "' and zone_country_id = '" . intval( $order->billing['country']['id'] ) . "' order by zone_id" );
  ...
  function after_process() {
    ...
    $emspay_order = $this->emspay->emsCreateApplePayOrder( $insert_id,
                                                             $order->info['total'], 
  ...
  function check() {
    if ( !isset( $this->_check ) ) {
      $check_query = tep_db_query( "select configuration_value from " . TABLE_CONFIGURATION . " where configuration_key = 'MODULE_PAYMENT_EMSPAY_APPLEPAY_STATUS'" );
  ...
  function install() {
    ...
    $sort_order = 0;
    $add_array = array(
      "configuration_title" => 'Enable EMS Online Apple Pay Module',
      "configuration_key" => 'MODULE_PAYMENT_EMSPAY_APPLEPAY_STATUS',
    ...
    $add_array = array(
      ...
      "configuration_key" => 'MODULE_PAYMENT_EMSPAY_APPLEPAY_ZONE',
    ...
    $add_array = array(
      ...
      "configuration_key" => 'MODULE_PAYMENT_EMSPAY_APPLEPAY_SORT_ORDER',
      "configuration_value" => 0,
  ...
  function keys() {
    return array(
      'MODULE_PAYMENT_EMSPAY_APPLEPAY_STATUS',
      'MODULE_PAYMENT_EMSPAY_APPLEPAY_ZONE',
      'MODULE_PAYMENT_EMSPAY_APPLEPAY_SORT_ORDER',
    );
  }
