...
class emspay_creditcard {
  ...
  // Class Constructor
  function emspay_creditcard() {
    global $order;

    $this->code = 'emspay_creditcard';
    $this->id = 'credit-card';
    $this->title_selection = MODULE_PAYMENT_EMSPAY_CREDITCARD_TEXT_TITLE;
    ...
    if ( (int)MODULE_PAYMENT_EMSPAY_ORDER_STATUS_ID > 0 ) {
      ...
      $payment = 'emspay_creditcard';
    } else if ( $payment=='emspay_creditcard' ) {
        $payment='';
      }
    if ( is_object( $order ) ) {
      $this->update_status();
  ...
  function update_status() {
    ...
    if ( ( $this->enabled == true ) && ( (int)MODULE_PAYMENT_EMSPAY_CREDITCARD_ZONE > 0 ) ) {
      $check_flag = false;
      $check_query = tep_db_query( "select zone_id from " . TABLE_ZONES_TO_GEO_ZONES . " where geo_zone_id = '" . intval( MODULE_PAYMENT_EMSPAY_CREDITCARD_ZONE ) . "' and zone_country_id = '" . intval( $order->billing['country']['id'] ) . "' order by zone_id" );
  ...
  function after_process() {
    ...
    if ( !is_array( $emspay_order ) or array_key_exists( 'error', $emspay_order) or $emspay_order['status'] == 'error' ) {
      ...
      $this->tep_remove_order( $insert_id, $restock = true );
        $reason = "Error placing Credit Cart order ";
  ...
  function check() {
    if ( !isset( $this->_check ) ) {
      $check_query = tep_db_query( "select configuration_value from " . TABLE_CONFIGURATION . " where configuration_key = 'MODULE_PAYMENT_EMSPAY_CREDITCARD_STATUS'" );
  ...
  function install() {
    ...
    $sort_order = 0;
    $add_array = array(
      "configuration_title" => 'Enable EMS Online Creditcard Module',
