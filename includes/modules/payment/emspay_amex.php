...
class emspay_amex {
  ...
  // Class Constructor
  function emspay_amex() {
    global $order;

    $this->code = 'emspay_amex';
    $this->id = 'amex';
    $this->title_selection = MODULE_PAYMENT_EMSPAY_AMEX_TEXT_TITLE;
  ...
  function update_status() {
    ...
    if ( ( $this->enabled == true ) && ( (int)MODULE_PAYMENT_EMSPAY_AMEX_ZONE > 0 ) ) {
      $check_flag = false;
      $check_query = tep_db_query( "select zone_id from " . TABLE_ZONES_TO_GEO_ZONES . " where geo_zone_id = '" . intval( MODULE_PAYMENT_EMSPAY_AMEX_ZONE ) . "' and zone_country_id = '" . intval( $order->billing['country']['id'] ) . "' order by zone_id" );
  ...
  function check() {
    if ( !isset( $this->_check ) ) {
      $check_query = tep_db_query( "select configuration_value from " . TABLE_CONFIGURATION . " where configuration_key = 'MODULE_PAYMENT_EMSPAY_AMEX_STATUS'" );
